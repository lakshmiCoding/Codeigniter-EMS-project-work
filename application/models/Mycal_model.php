...
class Mycal_model extends CI_Model{
    ...
    public function __construct(){
        ...
        {cal_cell_end}</td></a>{/cal_cell_end}
...
';
    ...
    public function getcalendar($year, $month){
        $this->load->library('calendar',$this->prefs); // Load calender library with the 'preferences set earlier'
        // $data = array(
        //     3  => "check",
        //     7  => "check1",
        //     13 => "bar",
        ...
        $query =  $this->db->select('date,content')->from('events')->like('date',"$year-$month",'after')->get();
    ...
    public function insert_events($data){
		$ins=$this->db->insert('events',$data);
		return $ins;
	}
